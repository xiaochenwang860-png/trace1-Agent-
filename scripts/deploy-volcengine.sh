...
if [[ "${ARK_API_KEY:-}" == "" \
  ...
  || "${APP_AUTH_TOKEN:-}" == "" \
  || "${TRACE_VIEWER_TOKEN:-}" == "" \
  || "${RECOVERY_OPERATOR_TOKEN:-}" == "" \
  || "${RECOVERY_OPERATOR_ID:-}" == "" ]]; then
  echo "ARK_API_KEY, ARK_MODEL, APP_AUTH_TOKEN, TRACE_VIEWER_TOKEN, RECOVERY_OPERATOR_TOKEN and RECOVERY_OPERATOR_ID are required in .env.production." >&2
  exit 1
fi
...
export TF_VAR_recovery_operator_token="$RECOVERY_OPERATOR_TOKEN"
export TF_VAR_recovery_operator_id="$RECOVERY_OPERATOR_ID"
