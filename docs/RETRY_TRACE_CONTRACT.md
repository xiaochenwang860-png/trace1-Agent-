...
这份文档是我留给回退模块开发同学的接入约定。

我已经完成的是**观测端**：稳定 ID、Attempt/Retry 事件、实时推送、Journal、持久化、前端展示和测试 Helper。真正的错误分类、重试次数、退避、备用模型或服务器切换仍由你的 Fallback Runner 实现。

> 你负责“是否重试、何时重试、换什么再执行”；我负责“把每次尝试实时、稳定、可关联地记录和展示”。

`apps/server/src/attempt-trace.ts` 中的 `AttemptTrace` 是正式预留接口，不是已经上线的回退策略。只有你把它包在真实 Runner 外层，生产执行才会产生 Attempt 和 Retry 事件。
...
| 事项 | Fallback Runner（你负责） | Glass Box（我负责） |
| --- | --- | --- |
| 错误分类 | 将异常归一为稳定 `errorCode` | 保存、脱敏并展示 |
| 是否重试 | 设置 `retryable`，执行策略判断 | 不替你做策略决策 |
...
你不需要直接操作 Live Trace、主 Store、Journal、HTTP 流或前端状态，也不要自行分配 `sequence`。
...
3. 切换备用 Runner 时仍使用同一 `workspacePath`；是否复用 `threadId` 由你的策略决定，但要有测试。
...
你不需要等待或清理 Journal。AgentService 的顺序是：
...
因此 Journal 慢磁盘不会拖住界面的终态，也不影响你判断是否继续回退。Journal 只恢复已经产生的观测记录，不会自动续跑任务。
...
如果你的策略确实需要修改这些边界，先和我对齐契约，不要在回退模块里另建一套 Trace 或状态机。
